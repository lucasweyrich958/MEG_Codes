...
data_predictor3 = data_predictor$X3
data_predictor4 = data_predictor$X4
  ...
  for (Y_current in 1: dimensions[2])
  {
    for (Z_current in 1: dimensions[3])
    {
      ...
      if (sum(pseudo_Ts_temp) == 0) { #I should be embarrased for forgetting the == !
        ...
        model.brain_model = aov(pseudo_Ts_temp ~ data_predictor1 + data_predictor2 + data_predictor3 + data_predictor4 + data_predictor1*data_predictor3) #the number indicates the column!
        ...
        F_value_temp_v1 = summary(model.brain_model)$coefficients["data_predictor1","F value"]
        
        F_stat_map_v1[X_current, Y_current, Z_current] = t_value_temp_v1
        
        #F_stat_map_v1[X_current, Y_current, Z_current] = t_value_temp_v1^2
...
#t_stat_output_name_v1 = paste('lm_regression_t_values_mainEffect', '_df_', df_1,sep = '')
...
F_stat_output_name_v1 = paste('lm_regression_F_values_mainEffect', '_df_', df_1, '_',df_2,sep = '')

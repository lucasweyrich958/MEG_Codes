function sourcespace_virtSens_findDominantOrientation_relative_CONN(selection_method,concat)

%PURPOSE:           Find the dominant orientation from the timeseries of each participant, and export this timeseries in ".swf" format.
%                   SPECIFIC TO "_relative" FUNCTION: Take the dominant orientation timeseries and baseline correct it.
%
...
%                   orientations
%                   04/20/2022  v4: Seth Springer edit: Baseline-correct
%                   the dominant orientation timeseries (edits marked with
%                   "SDS")
...
clear inputs defaults answer

%prompt for baseline inputs and convert inputs to number format% SDS
inputs = {'Baseline Time Start', 'Baseline Time End'};
defaults = {'0', '0'};	
answer = inputdlg(inputs, 'Please Input Parameters', 2, defaults, 'on');
[baseline_timestart,baseline_timeend] = deal(answer{:});
baseline_timestart =str2num(baseline_timestart);
baseline_timeend =str2num(baseline_timeend);
...
timeend = ((timeend-epochstart)/timesample);

%find the cell references for the baseline window% SDS
baseline_timestart = ((baseline_timestart-epochstart)/timesample)+1;
baseline_timeend = ((baseline_timeend-epochstart)/timesample);
 ...
 %       if ori1_Max > ori2_Max
  %          Dom_Ori = ori_timeseries(1,:);
   %         dominant_orientations(1,i) = 1;
    ...
    %Now we need to baseline correct the dominant orientation vector (SDS)
    baseline_average = mean(Dom_Ori(baseline_timestart:baseline_timeend));
    
    Dom_Ori_REL = (Dom_Ori-baseline_average)/baseline_average;
    ...
    filename = strcat(name,'_sourcespace_virtSens_findDominantOrientation_relative','.swf');
    fid = fopen(filename,'wt');
    fprintf(fid,'%s\n',header);
    fprintf(fid,'%s','Dom-Ori:   ');   
    fclose(fid);
    dlmwrite(filename,Dom_Ori_REL,'delimiter',' ','-append');
    ...
    if nargin == 2 & strcmp(concat,'y')
        if i == 1
            Dominant_Ori_Concat = Dom_Ori_REL;
        elseif i > 1
            Dominant_Ori_Concat = [Dominant_Ori_Concat; Dom_Ori_REL];
        end
...
if nargin == 2 & strcmp(concat,'y')
    dlmwrite('Concatentated_DomOri_Timeseries_relative.txt',Dominant_Ori_Concat,'delimiter','\t','precision',4)
    dlmwrite('Averaged_DomOri_Timeseries_relative.txt',Dominant_Ori_Concat_Average,'delimiter','\t','precision',4)
end
...
fid = fopen('Dominant Orientations_relative.txt','wt');

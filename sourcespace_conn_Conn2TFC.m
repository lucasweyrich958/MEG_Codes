...
InputFolder = uigetdir('D:\','Please Select Input Directory'); %static
InputFilenames = uigetfile('*.conn','Please Select CONNs to Convert','Multiselect','on');
%ImportFilenames{end+1} = '<filename #2>';
%ImportFilenames{end+1} = '<filename #3>';
% Export folder
% Note: Appendix ' - Export' and extension .tfc/.bsa will be added
% automatically
OutputFolder = uigetdir('D:\','Please select output directory'); %static path, change later
...
for CntFile=1:length(InputFilenames)
    ...
    % Load results
    ConnResults = readBESAconn(CurrFilePathName);
